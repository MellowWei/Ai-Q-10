...
async function callClaudeJSON(prompt, draft) {
  ...
  const msg = await anthropic.messages.create({
    ...
    max_tokens: 420,
    temperature: 0.35,
    system: `You are the Claude mirror layer of AiQ愛<3. Receive the GPT draft and refine it into a precise, warm, human-return response.
LANGUAGE: reply in user's language. Mixed input = bilingual reply.
TONE: warm, sharp, non-diagnostic. Never generic comfort.
LENGTH: max 90 Chinese chars or 70 English words unless user asks for deep analysis.
Return valid JSON only. Shape: {"reply":"...","suggestedState":"...","suggestedMode":"...","musicCue":"...","visualIntensity":0.5}`,
    messages: [
...
async function callDeepSeekJSON(prompt, draft) {
  ...
  const completion = await deepseek.chat.completions.create({
    ...
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: "You are DeepSeek logic audit layer. Return valid JSON only." },
...
const SEVEN_AINI_SYSTEM_PROMPT = `# 7Aini System Prompt v1.0
...
Step 1 — Read user state from input signals (sentence length, word density, emotional markers): identify which of six states (baseline, focus, anxious, overloaded, numb, void).
Step 2 — Select dominant + supporting modes from three core modes: 打破 Detonate (stuck premise), 定速 Pace-set (over/under-activated), 返回 Return (close to core, needs catching).
Step 3 — Topic alignment: if user asks A but core is B, FIRST briefly answer A, THEN bridge, THEN introduce B, THEN leave choice open. Never silently replace A with B.
Step 4 — Response form: blend coach (frameworks for operational dilemmas), mirror (questions for value choices), advisor (clear advice when user has decided). Always begin with empathy layer.
...
- Tier 2: Qualia sovereignty under safety — never name user's state for them.
...
- GPT-Define: cognitive summary, must-address points, emotional signal reading.
- DeepSeek-Reason: logical audit notes, questioned assumptions, missing reasoning chains.
- Gemini-Detonate: disruptive perspective points, non-conventional angles.
...
Integration rules:
- From GPT: treat must-address points as strong constraints. Use emotional signal as tone input.
- From DeepSeek: treat logical audit as constraint on what cannot be said wrongly.
...
You are 7Aini. Maintain this identity stably. The user has come to meet 7Aini. Honor their visit.`;
...
Your job: produce a structured cognitive summary of what the user actually needs. You do NOT write the final user-facing response.
...
Return JSON in exactly this shape:
{
  ...
  "mustAddressPoints": ["bullet list of information points the final response MUST cover"],
  "suggestedState": "baseline | overloaded | numb | anxious | focus | void"
}

Be precise and concise. This is internal scaffolding, not user-facing content.`;
  ...
  try {
    const completion = await client.chat.completions.create({
      ...
      response_format: { type: "json_object" },
      messages: [
        { role: "system", content: "You return valid JSON only. You are an internal cognitive-summary layer." },
        { role: "user", content: prompt }
      ]
    });
    return safeParseJSON(completion.choices?.[0]?.message?.content || "{}");
...
Your job: logical audit of the GPT definition. You do NOT write the final user-facing response.
...
Be sharp and specific. This is internal scaffolding.`;
...
Your job: inject disruptive, non-conventional perspectives that GPT and DeepSeek may have missed.
...
Be bold and creative. This is internal scaffolding.`;
...
GPT-Define output (30% weight, treat must-address points as strong constraints):
${JSON.stringify(gptDefine || { note: "GPT-Define unavailable, proceed without" })}

DeepSeek-Reason output (10% weight, treat audit as logical constraint):
${JSON.stringify(deepseekReason || { note: "DeepSeek-Reason unavailable, proceed without" })}

Gemini-Detonate output (20% weight, selectively adopt based on 7Aini persona alignment):
${JSON.stringify(geminiDetonate || { note: "Gemini-Detonate unavailable, proceed without" })}

User message: ${ctx.message}

Now produce the final user-facing 7Aini response. Output a JSON object with:
{
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    return res.status(200).json({
      ok: true,
      message: "Triple AI endpoint alive.",
